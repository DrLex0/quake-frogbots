...
9. **Special path modes.** You can apply these while making the paths, or afterwards. The modes for a marker's paths can be seen by pressing the `R` key.  
   ...
   - **Jump ledge** (shown as `‘J’`, number 1024 in code) is to explicitly mark the path as requiring a jump, usually to _get up onto ledges._ The usefulness of this mode has become limited, now that the bot can in most cases figure out by itself when it needs to jump to cross a gap. Only paths going up a step taller than 18 units, _must_ be marked with either this or precise jump mode to ensure the bot will jump onto the step.  
